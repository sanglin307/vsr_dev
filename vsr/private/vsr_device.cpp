...
{
	_running = true;
	_pthread = new std::thread([](vsrStage* s) { s->Process(); }, this);
}
...
vsrStage_InputAssemble::vsrStage_InputAssemble() 
{
}

void vsrStage_InputAssemble::Process()
{
	while (_running)
	{
...
vsrStage_VertexShader::vsrStage_VertexShader()
{
}

void vsrStage_VertexShader::Process()
{
	while (_running)
	{
		std::this_thread::yield();
...
{
	auto *ia = new vsrStage_InputAssemble;
	_stages.push_back(ia);
	_stages.push_back(new vsrStage_VertexShader);
}
